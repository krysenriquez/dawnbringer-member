...
} from '@/features/account/stores/AddressInfoQueryProvider'
import {updateAddress} from '@/features/account/api'
...
const EditAddressForm = () => {
  ...
  const addressInfo = useAddressInfoQueryData()
  const isLoading = useAddressInfoQueryLoading()
  ...
  const {
    formId,
    formField: {label, address1, address2, city, zip, province, country, addressType, isDefault},
  } = addressFormModel

  useEffect(() => {
    if (addressInfo && !isLoading) {
      setInitialAddress((prevState) => {
        return {
          ...prevState,
          ...addressInfo,
        }
    ...
    swal
      ...
      .then(async (result) => {
        if (result.isConfirmed) {
          ...
          try {
            const {data: response} = await updateAddress(values)
            swal.fire('Address Updated!', response.detail, 'success')
            console.log()
          } catch (ex) {
