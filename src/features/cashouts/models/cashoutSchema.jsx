...
import {getWalletCanCashout, getWalletMaxCashout} from '../api'

const validateWalletCashoutSchedule = async (ctx) => {
  return await getWalletCanCashout({
    ...
    .catch((err) => {
      return ctx.createError({path: 'wallet', message: err.response.data.detail})
    })
}
...
const validateWalletMaxCashout = async (ctx) => {
  return await getWalletMaxCashout({
    ...
    .catch((err) => {
      return ctx.createError({path: 'activityAmount', message: err.response.data.detail})
    })
}
